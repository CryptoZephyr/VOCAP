...
- The latest immutable RC.5 proof preflight at block `14,026,562` with proving block `14,026,552` received HTTP `200` from both health and both OHTTP-key endpoints, returned `9` proof facts, `18` `apply_actions` words, and `304304` proof bytes, and stopped before fee submission because the pool allowance is `0`. The same-day service refresh exposed no immutable revision or image digest.
...
- A fresh RC.5 proof preflight at block `14,027,254` with proving block `14,027,244` again received HTTP `200` from both health and both OHTTP-key endpoints and returned `9` proof facts, `18` `apply_actions` words, and `304304` proof bytes. The fee estimate stopped before submission because the pool allowance is `0`.
- An exploratory comparison compiled immutable RC.2 source at commit `9bfeb8dd35565a2915a0617dff3f649bd5bb891a` with the available dependency tree and produced the same non-broadcast Mainnet proof shape. This is compatibility evidence only, not a full RC.2 package verification or service image-digest proof.
- An isolated RC.2 worktree with a fresh dependency install passed the SDK build and all `252` non-devnet tests across `26` test files. Its proof preflight at block `14,027,816`, proving block `14,027,806`, returned `9` proof facts, `18` `apply_actions` words, and `312200` proof bytes with all four service endpoints at HTTP `200`. The fee estimate stopped because the pool allowance is `0`. The live service image revision remains unverified.
- A fresh RC.5 proof preflight at block `14,028,015` with proving block `14,028,005` again returned `9` proof facts, `18` `apply_actions` words, and `306596` proof bytes with all four service endpoints at HTTP `200`. The fee estimate stopped because the pool allowance is `0`.
