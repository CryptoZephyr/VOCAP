...
- [x] Confirm retry, shutdown, and health behavior through the Sepolia supervised smoke test and three successful Mainnet GitHub Actions catch-up runs (`33249047016`, `33249526648`, `33251549194`).
...
- [x] Execute the migration step with the protected migration role and verify the runtime role has its required application privileges. Runs `33249047016`, `33249526648`, and `33251549194` passed the five-table privilege check.
...
- Operational Mainnet runs: the compiled indexer passed the preflight and migration checks, verified the restricted runtime role, and completed three successful bounded catch-up runs. Run `33249047016` processed the first 12 chunks from block `14042348`; follow-up run `33249526648` processed the next 12 chunks through block `14043547`; final catch-up run `33251549194` processed 12 more chunks from block `14043548` through `14044147`. All three runs reported zero execution failures. The scheduled job continues from the persisted cursor.
