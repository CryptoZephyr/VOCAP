...
const FAQ_ITEMS = [
  ...
  ["What can I inspect on Mainnet?", "The Capability Trace shows the three recorded Router receipts, their block numbers and actual fees, plus the public STRK20 → Router → target → RETURN boundary."],
  ...
  ["Where can I inspect the proof?", "Open the Capability Trace to review the recorded Mainnet lifecycle and follow each receipt on Starkscan. It’s a read-only evidence surface."],
] as const;
    ...
    <div className="border-t border-border bg-background text-foreground">
      <nav className="sticky top-0 z-20 border-b border-border bg-background/95 px-4 py-3 backdrop-blur md:px-10" aria-label="VOCAP sections">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-5 gap-y-2 text-xs font-semibold uppercase tracking-[0.12em]">
          ...
          <Button className="ml-auto h-8 px-3 text-xs" nativeButton={false} render={<a href={ROUTES.playground} onClick={(event) => handleInternalClick(event, ROUTES.playground)} />}>Capability Trace</Button>
        </div>
      </nav>
